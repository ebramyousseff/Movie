...
const TVDetails = () => {
  ...
  const { tvCast, loading, error } = useSelector((state) => state.tvCast);

  useEffect(() => {
    if (tvID) {
      dispatch(fetchDetails({ kind: "tv", id: tvID }));
      dispatch(fetchCastData({kind : "tv" ,id : tvID}));
      window.scrollTo(0, 0);
    }
  }, [tvID, dispatch]);
